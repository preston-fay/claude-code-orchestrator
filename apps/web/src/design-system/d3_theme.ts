...
/**
 * Get label style for readable overlay text on maps/charts
 * Includes stroke halo for readability on any background
 */
...
/**
 * Get spot color outline style for emphasis
 */
export function spotOutline(theme: Theme): Record<string, string | number> {
  const colors = getThemeColors(theme);

  return {
    stroke: colors.spotColor,
    strokeWidth: '3px',
    strokeDasharray: '5,3',
    fill: 'none',
    opacity: 0.9,
