"""Orchestrator run-loop core implementation."""
...
from .utils import is_agent_error_retryable
from .detectors import AgentDetectorMixin
from .transitions import PhaseTransitionMixin


class Orchestrator(AgentDetectorMixin, PhaseTransitionMixin):
    ...
    async def _invoke_agent_async(
        ...
        try:
            ...
            exec_result = await retry_async(
                execute_with_retry,
                config=retry_cfg,
                is_retryable=lambda e, _: is_agent_error_retryable(e, retry_cfg),
            )
